...
const docSchema = z.object({
  ...
  // รับจากฟอร์ม แต่จะเอาไป pack เป็น description
...
/** แปลง tags + body จากฟอร์ม → description ที่เก็บใน DB */
function buildDescriptionFromForm(input: {
  tags?: string | null;
  body?: string | null;
}): string {
  const parts: string[] = [];
...
// list docs พร้อมนับ chunks + botKnowledges
r.get("/docs", async (req: Request, res: Response) => {
  try {
    const tenant = tenantOf(req);
    const page = pageNum(req.query.page);
    const pageSize = limitNum(req.query.pageSize);
    ...
    const [items, total] = await Promise.all([
      ...
      prisma.knowledgeDoc.count({ where: { tenant } }),
    ]);
...
r.post("/docs", async (req: Request, res: Response) => {
  try {
    ...
    const description = buildDescriptionFromForm({ tags, body });
...
r.get("/docs/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (!item)
      return res
        .status(404)
        .json({ ok: false, message: "not_found" });
...
r.patch("/docs/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      data.description = buildDescriptionFromForm({
...
r.get("/docs/:id/chunks", async (req: Request, res: Response) => {
  try {
    ...
    if (!doc)
      return res
        .status(404)
        .json({ ok: false, message: "doc_not_found" });
...
r.patch("/chunks/:chunkId", async (req: Request, res: Response) => {
  try {
    ...
    if (!chunk || chunk.doc.tenant !== tenant)
      return res
        .status(404)
        .json({ ok: false, message: "chunk_not_found" });
